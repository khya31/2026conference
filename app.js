    ...
    function renderInsuranceSurvey() {
      ...
      el.insuranceParticipants.innerHTML = cards.map((card, index) => {
        ...
        const guardianFields = child ? `<div class="guardian-fields form-grid" data-guardian-fields ${insured ? '' : 'hidden'}>
          ...
          <label class="field"><span>監護人身分證字號</span><input data-insurance-field="guardianNationalId" maxlength="10" pattern="[A-Za-z][1289][0-9]{8}" autocapitalize="characters" placeholder="例：A123456789" value="${escapeHtml(data.guardianNationalId)}" ${insured ? 'required' : ''}></label>
        </div>` : '';
        return `<article class="insurance-member" data-member-id="${memberId}" data-child="${child}">
          ...
          <div class="insurance-details form-grid" data-insurance-details ${insured ? '' : 'hidden'}>
            ...
            <label class="field"><span>被保險人身分證字號</span><input data-insurance-field="insuredNationalId" maxlength="10" pattern="[A-Za-z][1289][0-9]{8}" autocapitalize="characters" placeholder="例：A123456789" value="${escapeHtml(data.insuredNationalId)}" ${insured ? 'required' : ''}></label>
            ${guardianFields}
          </div>
        </article>`;
      }).join('');
